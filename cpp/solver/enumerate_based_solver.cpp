...
EnumerateBasedCompositionSolver* current = nullptr;

void EnumerateBasedCompositionSolver::getMoreComponents() {
...
HASH EnumerateBasedCompositionSolver::getComponentHash(int k) {
...
void EnumerateBasedCompositionSolver::insertInput(Example* input) {
    auto feature = util::dataList2String(*input);
    inp_pool[feature] = input;
}

double EnumerateBasedCompositionSolver::evaluateOverfitRate(const std::vector<Program *> &program) {
    std::set<std::string> S;
    for (auto item: inp_pool) {
        auto* inp = item.second;
        DataList oup = task->execute(program, *inp);
        auto feature = util::dataList2String(oup);
        S.insert(feature);
    }
    return 1.0 * S.size() / inp_pool.size();
}

double EnumerateBasedCompositionSolver::evaluateOverfitRate(EnumerateInfo* info) {
    return evaluateOverfitRate(getProgramList(info));
}

void EnumerateBasedCompositionSolver::addCounterExample(std::pair<Example *, Example *> counter_example) {
...
void EnumerateBasedCompositionSolver::initNewProgram() {
...
EnumerateBasedCompositionSolver::EnumerateBasedCompositionSolver(Task *_task):
...
bool EnumerateBasedCompositionSolver::isAddExpression(int k, EnumerateInfo* x) {
...
std::pair<EnumerateInfo*, EnumerateInfo*> EnumerateBasedCompositionSolver::recoverResult(EnumerateInfo *info) {
    for (auto& infos: info_list) {
        for (auto* x: infos) {
...
std::pair<EnumerateInfo*, EnumerateInfo*> EnumerateBasedCompositionSolver::addExpression(int k, EnumerateInfo *info) {
...
std::pair<EnumerateInfo*, EnumerateInfo*> EnumerateBasedCompositionSolver::getNextComposition(int k) {
    int l_id = 0, r_id = 0;
...
std::vector<Program *> EnumerateBasedCompositionSolver::getProgramList(EnumerateInfo *info) {
    if (!info) return {};
...
std::vector<Program *> EnumerateBasedCompositionSolver::synthesisFromExample() {
...
void EnumerateBasedCompositionSolver::getInputStatus(const std::vector<Program*>& candidate) {
    std::set<std::string> S;
    for (auto* example: task->example_space->input_space) {
        DataList feature = task->execute(candidate, *example);
        std::string str_feature = util::dataList2String(feature);
        S.insert(str_feature);
    }
    int num = S.size(), tot = task->example_space->input_space.size();
    LOG(INFO) << "Different Input Num " << num << "/" << tot << " = " << double(num) / tot;
}

void EnumerateBasedCompositionSolver::getExampleStatus(const std::vector<Program *> &candidate_result) {
    std::map<std::string, Example*> M;
    for (auto& counter_example: example_list) {
        std::string feature = util::dataList2String(*counter_example.first);
        M[feature] = counter_example.first;
        feature = util::dataList2String(*counter_example.second);
        M[feature] = counter_example.second;
    }
    std::set<std::string> S;
    for (auto& item: M) {
        DataList feature;
        for (const Data& inp: *item.second) {
            for (auto* p: candidate_result)
                feature.push_back(p->run({inp}));
        }
        std::string str_feature = util::dataList2String(feature);
        S.insert(str_feature);
    }
    int num = S.size(), tot = M.size();
    LOG(INFO) << "Different Example Input Num " << num << "/" << tot << " = " << double(num) / tot;
}

std::pair<Example *, Example *> EnumerateBasedCompositionSolver::verifyOccam(const std::vector<Program *> &program_list) {
    data_map.clear();
    ...
    auto verify = [&](int pos)->std::pair<Example*, Example*>{
        ...
        for (int i = 0; i < complete_program_list.size(); ++i) {
            if (is_cached[i] == -1) {
                ...
                DataList result = task->execute(complete_program_list[i], *example_info.example);
...
std::vector<Program*> EnumerateBasedCompositionSolver::synthesis() {
    int turn_num = 0;
    while (true) {
        turn_num += 1;
