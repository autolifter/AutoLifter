...
void Solver::acquireMoreExample(int target_num) {
    ...
    for (int i = pre_size; i < example_num; ++i) {
        ...
        auto result = task->executeExample(*example);
        std::string feature = util::dataList2String(result.first);
        example_structure.push_back({feature, result.second, example});
    }
}

Solver::Solver(Task *_task): task(_task), example_num(_task->example_space->c.init_size) {
    start_verify_pos = 0;
